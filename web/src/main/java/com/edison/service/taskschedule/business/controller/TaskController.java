...
import com.edison.service.taskschedule.business.valid.TaskValidator;
import com.edison.service.taskschedule.business.vo.TaskVO;
import com.edison.services.orgcenter.client.organization.StationClient;
...
public class TaskController {
	 ...
	 * @param idArray
	 */
	@ApiOperation(value = "删除", notes = "删除任务", httpMethod = "POST")
	@PostMapping("/deleteTask")
  	@BusinessFuncMonitor(value = "taskschedule.task.task.delete")
	public Map delete(@ApiParam(required = true,value = "任务id") @RequestParam Long id,@ApiParam(required = true,value = "用户id")@RequestParam Long uid){
	...
	public	TaskVO update(@RequestBody @Valid TaskEditDto taskEditDto, @RequestParam Long id){
		Task task = new Task();
	...
	public List<Map> findByDate(@ApiParam(required = true) @RequestParam Date beginAt,@ApiParam(required = true) @RequestParam String uid){
	...
	public TaskVO updateRespUid(@ApiParam(value = "任务id")@RequestParam Long id,@ApiParam(value = "被指派人id") @RequestParam String respUid){
	 ...
	 * @return
	 */
	@ApiOperation(value = "查询", notes = "根据用户与任务id查看任务", httpMethod = "GET")
	@GetMapping("/personTask")
	public TaskVO personTask(@ApiParam(value = "任务id") @RequestParam Long id,@ApiParam(value = "用户id")@RequestParam String uid){

		Task task = taskService.prosonTask(id, uid);
		TaskVO taskVO = initViewProperty(task);
		return taskVO;
	}
	 ...
	 * 设置任务提醒时间
	 */
	@ApiOperation(value = "修改", notes = "设置任务提醒时间", httpMethod = "GET")
	@PostMapping("/updateTaskReminderTime")
	public TaskVO updateTaskReminderTime(@ApiParam(required = true,value = "任务id")@RequestParam Long id,@RequestParam Date date){
		Map<Boolean,String> map=new HashMap<>();
	 ...
	 * @return
	 */
	@ApiOperation(value = "查询", notes = "查询我关注的任务PC", httpMethod = "POST")

	@PostMapping("/findMyAttention")
	public PageContent<TaskVO> findMyAttention(@RequestBody PageSearchRequest<TaskCondition> pageSearchRequest){
		TaskCondition searchCondition = pageSearchRequest.getSearchCondition();
		Long uid = Long.parseLong(searchCondition.getUid());
		List<Long> taskId = attentionTaskService.findTaskId(uid);
		PageRequest pageRequest = PageRequestConvert.convert(pageSearchRequest);
		List<TaskVO> voList = new ArrayList<>();
		Page<Task> tasks =null;
		Long taskStatus = searchCondition.getTaskStatus();
		for (Long id : taskId) {
			searchCondition.setTaskId(id);
			 tasks = taskService.find(searchCondition, pageRequest);
		}
		if (taskStatus!=null){
			for (Task task : tasks) {
				Long tId = task.getId();
				Taskprogress taskprogress = taskprogressService.findByCteateAndStatus(tId, taskStatus);
				TaskVO taskVO = initViewProperty(task);
				if (taskprogress!=null) {
					taskVO.setTaskProgress(taskprogress.getTaskProgress());
				}
